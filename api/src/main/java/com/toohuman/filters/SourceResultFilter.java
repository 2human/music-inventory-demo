...
//				System.out.println("NaN entered as ID");
	...
	public static Set<Sources> getFilteredByMSEntriesSet(String msEntries, Set<Sources> resultSet){
		Set<Sources> workingSet = new HashSet<Sources>();
		for(Sources result: resultSet) {
			if(result.getMsEntries().toLowerCase().indexOf(msEntries.toLowerCase()) != -1) workingSet.add(result);			
		}
		return workingSet;		
	}	
...
//				System.out.println("NaN entered as sourceNumber");
			...
			if(curResult.getMsEntries().toLowerCase().indexOf(keyword.toLowerCase()) != -1) workingSet.add(curResult);
		}
		return workingSet;
	}
