...
describe('searchReducer', () => {
  ...
  it('returns a default state for an undefined existing state', () => {
    expect(searchReducer(undefined, {})).toEqual({
      ...
      status: undefined,
      error: false,
      resultsPerPageOptions: [10, 25, 100, 500],
      maxPageButtons: 8,
      resultsPerPage: 25,
      currentPage: 1,
  ...
  describe('searchSort', () => {
    ...
    it('sorts the data according to the sortBy props column', () => {
      ...
      ).toEqual(sortedResults(results, sortBy));
    });
  });
  ...
  describe('updateResults', () => {
    ...
    it('updates the edited result when updateType is "edit"', () => {
      ...
      const newResults = searchReducer(
        { results },
        updateResults('edit', editedRow)
      );
    ...
    it("adds a new result when update type is 'create'", () => {
      const createdRow = { id: 997, field1: 'newvalue' };
      const newResults = searchReducer(
        { results },
        updateResults('create', createdRow)
      ).results;
      expect(newResults).toHaveLength(results.length + 1);
      expect(newResults[2].id).toEqual(createdRow.id);
    });

    it('does not add a new result when viewing a different datatype', () => {
      const sourceResults = [
        { id: 998, callNumber: 'callnum1' },
        { id: 999, callNumber: 'callNum2' },
      ];
      const createdRow = { id: 997, melodicIncipit: 'melincip' };
      const newResults = searchReducer(
        { results: sourceResults, dataType: 'sources' },
        updateResults('create', createdRow)
      ).results;
      expect(newResults).toHaveLength(sourceResults.length);
    });
  });
});
