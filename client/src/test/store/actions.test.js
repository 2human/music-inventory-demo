...
describe('actions', () => {
  ...
  describe('modal', () => {
    ...
    it('openSearchGuide returns the right action type', () => {
      expect(openSearchGuide()).toMatchObject({
        type: MODAL_OPEN_SEARCH_GUIDE,
      });
    });

    it('openCitation returns the right action type', () => {
      expect(openCitation()).toMatchObject({
        type: MODAL_OPEN_CITATION,
      });
    });

    it('openBookPreview returns the right action type', () => {
      expect(openBookPreview()).toMatchObject({
        type: MODAL_OPEN_BOOK,
      });
    });

    it('openDisclaimer returns the right action type', () => {
      expect(openDisclaimer()).toMatchObject({
