...
import { createContainerWithStore } from '../test-utils/domManipulators';
import { fetchResponseOk } from '../test-utils/spyHelpers';
import {
  advancedFields,
  basicFields,
  replicateFormInputs,
...
import { itMapsStateToPropsWhenNoOwnProps } from '../test-utils/connectorHelpers';
...
describe('ConnectedSearchForm', () => {
  ...
  describe('mapStateToProps', () => {
    ...
    it('maps the openSearchGuide action to the onGuideClick prop', () => {
      expect(mapDispatchToProps).toMatchObject({
        onGuideClick: openSearchGuide,
      });
    });

    it('maps the openDisclaimer action to the onDisclaimerClick prop', () => {
      expect(mapDispatchToProps).toMatchObject({
