...
describe('SearchForm', () => {
  ...
  describe('searchGuideBtn', () => {
    it('displays the #searchGuideBtn element', () => {
      render(<SearchForm />);
      expect(element('#searchGuide.btn-text')).not.toBeNull();
    });

    it('triggers open guide when clicked', () => {
      const guideClickSpy = jest.fn();
      render(<SearchForm onGuideClick={guideClickSpy} />);

      const guideClickBtn = element('#searchGuide.btn-text');
      click(guideClickBtn);

      expect(guideClickSpy).toHaveBeenCalled();
    });
  });
