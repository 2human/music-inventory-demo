...
import { NavLinkList } from '../layouts/Navigation/NavLinkList';
...
describe('NavLinkList', () => {
  ...
  it('renders the navlink-list nav element', () => {
    render(<NavLinkList />);
    expect(element('nav.navlink-list')).not.toBeNull();
  });

  it('renders the navigation link list container', () => {
    render(<NavLinkList />);
    expect(
      element('nav > ul.navlink-list__container')
    ).not.toBeNull();
  });

  it('renders a list item for each link', () => {
    render(<NavLinkList links={links} />);
    expect(elements('li.navlink-list__item')).toHaveLength(
      links.length
    );
  });
  ...
  it('renders an navlink anchor element within each list item', () => {
    render(<NavLinkList links={links} />);
    const renderedLinks = elements(
      '.navlink-list__item a.navlink-list__link'
    );
